...
async function openTab(
  ...
  await page.goto(URL);
  await page.waitForFunction(() => (window as any).__moltenReady === true, {
    timeout: 10_000,
  });
  return page;
}
...
const db = {
  set: (page: Page, col: string, key: string, val: unknown) =>
    page.evaluate(([c, k, v]) => (window as any).dbSet(c, k, v), [
      col,
      key,
      val,
    ] as const),
  ...
  delete: (page: Page, col: string, key: string) =>
    page.evaluate(([c, k]) => (window as any).dbDelete(c, k), [
      col,
      key,
    ] as const),
  role: (page: Page) => page.evaluate(() => (window as any).dbRole()),
};
...
test("each of 3 tabs writes 20 entries — all 60 readable from leader", async ({}, testInfo) => {
  ...
  await Promise.all([
    ...Array.from({ length: 20 }, (_, i) =>
      db.set(tab1, "bulk", `t1-${i}`, { src: 1, i })
    ),
    ...Array.from({ length: 20 }, (_, i) =>
      db.set(tab2, "bulk", `t2-${i}`, { src: 2, i })
    ),
    ...Array.from({ length: 20 }, (_, i) =>
      db.set(tab3, "bulk", `t3-${i}`, { src: 3, i })
    ),
  ]);
...
test("follower is promoted to leader when the leader tab closes", async ({}, testInfo) => {
  ...
  await tab2.waitForFunction(() => (window as any).dbRole() === "leader", {
    timeout: 8_000,
  });
...
test("third tab becomes follower of the promoted leader", async ({}, testInfo) => {
  ...
  await tab2.waitForFunction(() => (window as any).__db.isLeader === true, {
    timeout: 5000,
  });
  await tab3.waitForFunction(() => (window as any).__db.isLeader === false, {
    timeout: 5000,
  });
  ...
  const roles = await Promise.all([
    tab2.evaluate(() =>
      (window as any).__db.isLeader ? "leader" : "follower"
    ),
    tab3.evaluate(() =>
      (window as any).__db.isLeader ? "leader" : "follower"
    ),
...
test("100 sequential writes from leader — all readable from follower", async ({}, testInfo) => {
  ...
  for (let i = 0; i < 20; i++) {
    ...
    const v = (await db.get(tab2, "seq", `k${idx}`)) as {
      index: number;
    } | null;
...
test("rapid concurrent writes from 2 followers do not corrupt data", async ({}, testInfo) => {
  ...
  await Promise.all([
    ...Array.from({ length: 30 }, (_, i) =>
      db.set(tab2, "race", `f2-${i}`, { i, src: 2 })
    ),
    ...Array.from({ length: 30 }, (_, i) =>
      db.set(tab3, "race", `f3-${i}`, { i, src: 3 })
    ),
  ]);
