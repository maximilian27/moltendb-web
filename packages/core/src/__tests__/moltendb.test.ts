...
describe("CRUD — leader", () => {
  ...
  it("getAll returns all documents in a collection", async () => {
    ...
    const all = (await db.getAll("items")) as unknown as Record<
      string,
      unknown
    >;
...
describe("terminate() and disconnect()", () => {
  ...
  it("disconnect() clears the sync timer", async () => {
    ...
    (db as unknown as { syncTimer: ReturnType<typeof setInterval> }).syncTimer =
      timer;
...
describe("Stress — bulk inserts (single leader)", () => {
  ...
  it("compact() between two write bursts does not corrupt data", async () => {
    ...
    await Promise.all(
      Array.from({ length: 200 }, (_, i) => db.set("cs", `k${i}`, { i }))
    );
    await db.compact();
    await Promise.all(
...
describe("Multi-tab parallel write stress", () => {
  it("leader + 2 followers writing 100 entries each — all 300 are readable", async () => {
    ...
    for (let i = 0; i < 100; i++) {
      expect(await leader.get("shared", `leader-${i}`)).toMatchObject({
        src: "leader",
        i,
      });
      expect(await leader.get("shared", `f1-${i}`)).toMatchObject({
        src: "f1",
        i,
      });
      expect(await leader.get("shared", `f2-${i}`)).toMatchObject({
        src: "f2",
        i,
      });
    }
  ...
  it("no request-ID collision when 3 tabs fire 200 ops concurrently", async () => {
    ...
    const all = await Promise.allSettled([
      ...Array.from({ length: 200 }, (_, i) =>
        leader.set("ids", `l${i}`, { i })
      ),
      ...Array.from({ length: 200 }, (_, i) =>
        follower1.set("ids", `f1${i}`, { i })
      ),
      ...Array.from({ length: 200 }, (_, i) =>
        follower2.set("ids", `f2${i}`, { i })
      ),
    ]);
